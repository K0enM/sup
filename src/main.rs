...
use core::fmt;
use std::{ffi::c_void, mem::{transmute, self}};
...
fn main() {
  unsafe {
    ...
    let IcmpCreateFile: IcmpCreateFile = transmute(GetProcAddress(h, "IcmpCreateFile\0".as_ptr()));
    ...
    let ip_options: IpOptionInformation = IpOptionInformation { ttl: 128, tos: 0, flags: 0, options_size: 0, options_data: 0 };
